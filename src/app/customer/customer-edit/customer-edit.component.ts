...
export class CustomerEditComponent implements OnInit {
  ...
  getCustomer(id: number) {
    this.dataService.getCustomer(id).subscribe((customer) => this.customer = customer);
  }

  submit() {
    if (this.customer.id === 0) {
      this.dataService.insertCustomer(this.customer).subscribe((insertedCustomer: ICustomer) => {
        if (insertedCustomer) {
          // Mark form a prisinte so that canDeactivateGuard won't prompt before navigation
          this.customerForm.form.markAsPristine();
          this.router.navigate(['/customers']);
        } else {
          const msg = 'Unable to instert customer';
  ...
  canDeactivate(): Promise<boolean> | boolean {
    ...
    const modalContent: IModalContent = {
      ...
      body: 'You have unsaved changes! Would youl like to leave the page and lose them?',
